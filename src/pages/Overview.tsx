/**
 * AgentLens — Overview Page
 * KPI cards + 24h summary chart
 */
...
export default function Overview() {
  const { metricsMap, allTxs, isLoading, chain, agents } = useApp();
  ...
  const kpis = useMemo(() => {
    ...
    const successRate =
      totalTx24h > 0
        ? ((success24h.length / totalTx24h) * 100).toFixed(1)
        : "—";

    const gasValues = success24h
      .map((t) => t.gas_quote ?? 0)
      .filter((v) => v > 0);
    const avgGasUsd =
      gasValues.length > 0
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      >
        <img
          src={heroBg}
          alt="AgentLens background"
          className="absolute inset-0 w-full h-full object-cover object-center opacity-30"
        />
        <div className="relative z-10 p-6 flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-1">
            <span className="badge-info text-[10px] font-semibold px-2 py-0.5 rounded-full">
              LIVE
            </span>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-neon leading-tight">
            AgentLens
          </h1>
          <p className="text-sm text-foreground-muted max-w-lg">
            On-Chain AI Agent Performance Analytics — powered by ERC-8004 identity
            registry &amp; Covalent GoldRush API.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          title="24h Agent Txs"
          value={isLoading ? "…" : kpis.totalTx24h.toLocaleString()}
          subValue="transactions last 24 hours"
        ...
        <KpiCard
          ...
          value={isLoading ? "…" : kpis.avgGasUsd === "—" ? "—" : `$${kpis.avgGasUsd}`}
          subValue="USD per successful tx"
          icon={<Zap size={14} />}
          accentColor="warning"
          loading={isLoading}
        />
        <KpiCard
          title="Active Agents"
          value={isLoading ? "…" : kpis.activeAgents}
          subValue={`from ERC-8004 registry on ${CHAIN_LABELS[chain]}`}
          icon={<Bot size={14} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-foreground-muted mt-0.5">
                Daily agent transaction success %
              </p>
            </div>
          </div>
          {isLoading ? (
            ...
            <EmptyChart message="No tx data yet — add your API key" />
          ) : (
        ...
        <div className="card-glow rounded-xl bg-background-card p-5">
          <div className="mb-4">
            <h3 className="text-sm font-semibold text-foreground">Tx Volume</h3>
            <p className="text-xs text-foreground-muted mt-0.5">Daily transactions</p>
          </div>
          {isLoading ? (
            <div className="h-48 bg-background-elevated rounded-lg animate-shimmer bg-[length:200%_100%]" />
          ) : allTxs.length === 0 ? (
            <EmptyChart message="Add API key to load data" />
          ) : (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={timeSeries} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                <defs>
      ...
      <div className="card-glow rounded-xl bg-background-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-foreground-muted mt-0.5">
              {CHAIN_LABELS[chain]} · sorted by transaction count
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-foreground-muted text-center py-8">
            No agent data — add your Covalent API key in Settings
          </p>
        ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-foreground-muted border-b border-border">
                  ...
                  <th className="text-right pb-2 font-medium hidden sm:table-cell">Avg Gas USD</th>
              ...
              <tbody>
                {topAgents.map((agent) => (
                  <tr
                    key={agent.address}
                    className="border-b border-border/40 table-row-hover cursor-pointer"
                    onClick={() => navigate("/leaderboard")}
                  >
                    <td className="py-2.5 pr-3">
                      <div>
                        <p className="font-medium text-foreground">{agent.name}</p>
                        <p className="text-foreground-subtle font-mono">
